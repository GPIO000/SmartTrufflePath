...
function generateAppIcons() {
  ...
  const iconDataUrl = `data:image/svg+xml;utf8,${svgIcon}`;
...
function drawCarMarker() {
  ...
  // Icona dell'Auto Vettoriale (Interamente Offline)
...
function loadSavedSellerData() {
  ...
  const savedCf = localStorage.getItem('seller_cf');
  if (savedName) document.getElementById('seller-name').value = savedName;
  if (savedCf) document.getElementById('seller-cf').value = savedCf;
}
...
function exportDataBackup() {
  const backupData = {
    ...
    clients: clients,
    tax_paid: isTaxPaid,
    car_coords: carCoords
  };

  const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(backupData));
  const downloadAnchor = document.createElement('a');
  downloadAnchor.setAttribute("href", dataStr);
  downloadAnchor.setAttribute("download", `trufflego_backup_${new Date().toISOString().slice(0,10)}.json`);
...
function importDataBackup(event) {
  ...
  fileReader.onload = function(e) {
    try {
      ...
      if (importedData.dogs) {
        dogs = importedData.dogs;
        localStorage.setItem('tartufo_dogs', JSON.stringify(dogs));
      }
      if (importedData.findings) {
        findings = importedData.findings;
        local